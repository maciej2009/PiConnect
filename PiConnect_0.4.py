...
class Piec():
        ...
        if  self.temp._czuj_buf_mid <= temp_zadana - histereza:     # piec pracuje
            ...
            if self.i % par['piec_delta_time'] == 0:            # pętla do mierzenia przyrostu/spadku temperatury
                ...
                if self.delta  <= -par['piec_delta_temp']:
                    self.zwieksz_moc(temp_zadana, histereza, cel)

                if self.delta  >= par['piec_delta_temp']:
                    self.zmniejsz_moc(temp_zadana, histereza, cel)
    ...
    def zwieksz_moc (self, temp_zadana, histereza, cel):
        ...
        if Piec._poziom == 0:
            self.on(temp_zadana, histereza, cel, 1)
        elif Piec._poziom == 1:
            self.on(temp_zadana, histereza, cel, 2)
        elif Piec._poziom == 2:
            self.on(temp_zadana, histereza, cel, 3)
        elif Piec._poziom == 3:
            self.on(temp_zadana, histereza, cel, 4)
        elif Piec._poziom == 4:
            self.on(temp_zadana, histereza, cel, 5)
        elif Piec._poziom == 5:
            self.on(temp_zadana, histereza, cel, 6)

        if akt_poziom < 6:
            Log().debug("[PIEC] - Zwiększono poziom z %s na %s (z %s kW na %s kW)" % ( akt_poziom, Piec._poziom, akt_moc, self._moc), cel)
        else:
            Log().debug("[PIEC] - Nie można już zwiększyć poziomu pracy pieca", cel)
            pass
...
class CO():
    ...
    def sprawdz_czy_wlaczyc(self):
        ...
        if par['okresgrzewczy'] == True:
            ...
            if self._status in (False, None):
                if self.temp._czuj_buf_mid < self._piec_temp_zadana - self._piec_histereza:
                    self.piec.on(self._piec_temp_zadana, self._piec_histereza,'CO')
