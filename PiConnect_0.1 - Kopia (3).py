...
def setup():
    ...
    def log_init():
            logging.basicConfig(filename='PiConnect.log', level=Params.log_loginglevel, format='%(levelname)s: %(asctime)s: %(message)s', datefmt='%Y-%m-%d %H:%M:%S %a')
            logging.info("[LOG] - Ustawiono poziom logowania: LoggingLevel = %s" % Params.log_loginglevel)
...
def loop():
    ...
    class Stycznik():
        ...
        def on(self):
            ...
            if self.wlaczony == False:
                print "[STYCZNIK] - Stycznik na GPIO nr %s został włączony" % self.stycz
                logging.info("[STYCZNIK] - Stycznik nr %s został włączony" % self.stycz)
            self.wlaczony = True

        def off (self):
            # Rpi: gpio.digitalWrite(stycz, gpio.LOW)
            if self.wlaczony == True:
                print "[STYCZNIK] - Stycznik na GPIO nr %s został wyłączony" % self.stycz
                logging.info("[STYCZNIK] - Stycznik nr %s został wyłączony" % self.stycz)
            self.wlaczony = False
    ...
    class Piec():
        ...
        def on (self, zrodlo, temp_pieca, okno_start, okno_stop, temp_co_cwu, histereza_co_cwu,  poziom = 1):   # źródło = CWU, CO lub dogrzanie do temp. min po wyjściu z petli.
            ...
            if zrodlo == 'CWU' and tmp_podgrzewam_CO is True:
                piec.off('CO',temp_pieca,okno_start, okno_stop,temp_co_cwu, histereza_co_cwu)

            if zrodlo == 'CO' and tmp_podgrzewam_CWU is True:
                piec.off('CWU',temp_pieca,okno_start, okno_stop,temp_co_cwu, histereza_co_cwu)

            # wyłacenie pieca jeżeli poziom ma ulec zmianie i piec działa
            if self.poziom > 0 and poziom <> self.poziom:
                piec.off(zrodlo, temp_pieca,okno_start, okno_stop,temp_co_cwu, histereza_co_cwu)
            ...
            if zmieniono is True:               # jeżeli nastapiła jakaś zmiana
                ...
                logging.info("[PIEC][%s] - Włączono grzanie na poziomie %s z mocą %s kW. Okno: %s - %s. Okres grzewczy: %s. Temp zbiornika: %s. Temp zadana: %s. Histereza: +/- %s." % (
                                    zrodlo, poziom, self.moc, okno_start, okno_stop, par['okresgrzewczy'], temp_pieca, temp_co_cwu, histereza_co_cwu))
        ...
        def off(self, zrodlo, temp_pieca, okno_start, okno_stop, temp_co_cwu, histereza_co_cwu):
            ...
            if self.podgrzewam_CWU is True:
                if zrodlo == 'CWU':
                    ...
                    logging.info("[PIEC][CWU] - Wyłączono grzanie. Okno: %s - %s. Okres grzewczy: %s. Temp zbiornika: %s. Temp zadana: %s. Histereza: +/- %s." % (
                                    okno_start, okno_stop, par['okresgrzewczy'], temp_pieca, temp_co_cwu, histereza_co_cwu))
            ...
            if wylacz_wszystkie_styczniki is True:
                st_grzalka_1 = Stycznik(par['st_grzalka_1'])
                st_grzalka_2 = Stycznik(par['st_grzalka_2'])
                st_grzalka_3 = Stycznik(par['st_grzalka_3'])
    ...
    class Zawor_Trojdrogowy():
        def __init__(self):
            pass
            self.temp_zasilania = 21




    class  Pompa_CO():
        def __init__(self, piec_temp):
            pass

            self.zaw_troj = Zawor_Trojdrogowy()
            self.temp_zasilania = self.zaw_troj.temp_zasilania
        ...
        def sprawdz_czy_wlaczyc(self):
            if temp.czuj_buf_mid >= self.temp_zasilania:    # jeżeli temperatura zbiornika jest wyższa od żądanej temp zasilania CO

                if (par['pom1_temp'] - par['pom1_hist']) > temp.czuj_wew_01:

                    if self.wlaczona == False:
                        print "[CO] - Włączam pompę CO. Temperatura pomieszczenia %s jest mniejsza od temp. zadanej %s" % (temp.czuj_wew_01, par['pom1_temp'])
                    self.on()

                else:
                    if self.wlaczona == True:
                        print "[CO] - Wyłączam pompę CO. Temperatura pomieszczenia %s jest większa od temp. zadanej %s" % (temp.czuj_wew_01, par['pom1_temp'])
                    self.off()
                ...
                if self.print_blokada_pompy_CO == False:
                    print "[CO] - Nie mogę włączyć pompy CO. Temperatura bufora %s jest za niska by uzyskać temp. zasilania %s" % (temp.czuj_buf_mid, self.temp_zasilania)
    ...
    while petla is True:
        ...
        if par['okresgrzewczy'] is False:       # jeżeli okres grzewczy=Nie to grzejemy tylko piec na potrzeby CWU
            ...
            if piec.podgrzewam_CWU is False:
                if cwu.status is True:              # jeżeli jest okno CWU
                    ...
                    if temp.czuj_buf_mid  <=  cwu.temp - cwu.histereza/2:  # jeżeli temperatura w środku BUFORA jest niższa od zadanej
                        #logging.info ("[CWU] - Włączam ogrzewnie CWU. Przycyzna: temp bufora %s < %s. Okres grzewczy: %s. Okno: %s -> %s." % (temp.czuj_buf_mid, cwu.temp - cwu.histereza/2, par['okresgrzewczy'], cwu.okno_start, cwu.okno_stop))
                        #print "[CWU] - Włączam ogrzewnie CWU. Przycyzna: temp bufora %s < %s. Okres grzewczy: %s. Okno: %s -> %s." % (temp.czuj_buf_mid, cwu.temp - cwu.histereza/2, par['okresgrzewczy'], cwu.okno_start, cwu.okno_stop)
                    ...
                    if temp.czuj_buf_mid  > cwu.temp + cwu.histereza/2:  # jeżeli temperatura w środku BUFORA jest wyższa od zadanej
                        #logging.info ("[CWU] - Wyłączam ogrzewanie CWU. Przycyzna: temp bufora %s > %s. Okres grzewczy: %s. Okno: %s -> %s." % (temp.czuj_buf_mid, cwu.temp + cwu.histereza/2, par['okresgrzewczy'], cwu.okno_start, cwu.okno_stop))
                        #print "[CWU] - Wyłączam ogrzewanie CWU. Przycyzna: temp bufora %s > %s. Okres grzewczy: %s. Okno: %s -> %s." % (temp.czuj_buf_mid, cwu.temp + cwu.histereza/2, par['okresgrzewczy'], cwu.okno_start, cwu.okno_stop)
                        piec.off('CWU',temp.czuj_buf_mid,cwu.okno_start, cwu.okno_stop,cwu.temp, cwu.histereza)

                else:
                    if piec.status is True:
                        #logging.info ("[CWU] - Wyłączam ogrzewanie CWU. Przycyzna: koniec okna nr %s. Okres grzewczy: %s. Okno: %s -> %s." % (cwu.okno, par['okresgrzewczy'], cwu.okno_start, cwu.okno_stop))
                        piec.off('CWU',temp.czuj_buf_mid,cwu.okno_start, cwu.okno_stop,cwu.temp, cwu.histereza)
            ...
            if cwu.status is False and piec.podgrzewam_CWU is True:
                if piec.status() is True:
                    #logging.info ("[CWU] - Wyłączam ogrzewanie CWU. Przycyzna: koniec okna nr %s. Okres grzewczy: %s. Okno: %s -> %s." % (cwu.okno, par['okresgrzewczy'], cwu.okno_start, cwu.okno_stop))
                    #print "[CWU] - Wyłączam ogrzewanie CWU. Przycyzna: koniec okna nr %s. Okres grzewczy: %s. Okno: %s -> %s." % (cwu.okno, par['okresgrzewczy'], cwu.okno_start, cwu.okno_stop)
                    piec.off('CWU',temp.czuj_buf_mid,cwu.okno_start, cwu.okno_stop,cwu.temp, cwu.histereza)
            ...
            if piec.podgrzewam_CWU is False:
                if co.status is True:        # jeżeli jest okno CO
                    if temp.czuj_buf_mid  <=  co.temp - co.histereza/2:  # jeżeli temperatura w środku BUFORA jest niższa od zadanej
                        #logging.info ("[CO] - Włączam ogrzewnie CO. Przycyzna: temp bufora %s < %s. Okres grzewczy: %s. Okno: %s -> %s." % (temp.czuj_buf_mid, co.temp - co.histereza/2, par['okresgrzewczy'], co.okno_start, co.okno_stop))
                    ...
                    if temp.czuj_buf_mid  > co.temp + co.histereza/2:  # jeżeli temperatura w środku BUFORA jest wyższa od zadanej
                        #logging.info ("[CO] - Wyłączam ogrzewanie CO. Przycyzna: temp bufora %s > %s. Okres grzewczy: %s. Okno: %s -> %s." % (temp.czuj_buf_mid, co.temp + co.histereza/2, par['okresgrzewczy'], co.okno_start, co.okno_stop))
            ...
            if co.status is False and piec.podgrzewam_CO is True:
                if piec.status is True:
                    #logging.info ("[CO] - Wyłączam ogrzewanie CO. Przycyzna: koniec okna nr %s. Okres grzewczy: %s. Okno: %s -> %s." % (co.okno, par['okresgrzewczy'], co.okno_start, co.okno_stop))
