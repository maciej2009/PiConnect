...
class Piec():
    ...
    def on (self, temp_zadana, histereza, poziom = 1):   # źródło = CWU, CO
        i = 0

        self.temp.readall()

        print "Podgrzej CWU   =",self._podgrzej_CWU
        print "Podgrzej CO    =",self._podgrzej_CO




        print self.i, "T.Buf: %s, T. zadana: %s, Hist: %s, T. pocz: %s, Delta: %s, Grzeję: %s" % (self.temp._czuj_buf_mid, temp_zadana, histereza, self.temp_pocz_zbiornika, self.delta, Piec._poziom)
    ...
    def zmniejsz_moc (self, temp_zadana, histereza):
        ...
        if akt_poziom >= 2 and akt_poziom <= 6:
            Log().debug("[PIEC] - Zmniejszam poziom z %s na %s (z %s kW na %s kW)" % (akt_poziom, Piec._poziom, akt_moc, self._moc), zrodlo)

        else:
            Log().debug("[PIEC] - Nie można już zmniejszyć poziomu grzania" , zrodlo)
...
class CWU():
    ...
    def sprawdz_czy_wlaczyc(self):
        ...
        if Piec._podgrzej_CWU == True:
            if Piec._poziom <> 0:
                self.piec.on(self._piec_temp_zadana,self._piec_temp_histereza,Piec._poziom)

            else:
                self.piec.on(self._piec_temp_zadana, self._piec_temp_histereza, 1)
...
class CO():
    ...
    def sprawdz_czy_wlaczyc(self):
        ...
        if par['okresgrzewczy'] == True:
            ...
            if self._status == True:
                Piec._podgrzej_CO = True

            if self._status == False:
                Piec._podgrzej_CO = False


            if Piec._podgrzewam_CWU == False and Piec._podgrzej_CO == True:
                if Piec._poziom <> 0:
                    self.piec.on(self._piec_temp_zadana, self._piec_histereza, Piec._poziom)


                else:
                    self.piec.on(self._piec_temp_zadana, self._piec_histereza, 1)



            #TODO: Wstawić ustalenie temperatury zasilania na podsatwie krzywej grzewczej. Ma znadac wartosć self._temp_zasilania_CO
            self.temp.readall()
            self._temp_zasilania_CO = 30
